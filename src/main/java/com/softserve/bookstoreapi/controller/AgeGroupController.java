...
public class AgeGroupController {
    ...
    @GetMapping("/api/ageGroups")
    public ResponseEntity<Page<AgeGroupDTO>> getAll(
            @PageableDefault(size = 10, sort = "name") Pageable pageable
    ) {
        Page<AgeGroupDTO> response = ageGroupService.getAllAgeGroups(pageable);
        return ResponseEntity.ok(response);
    }
    @DeleteMapping("/api/ageGroups/{id}")
